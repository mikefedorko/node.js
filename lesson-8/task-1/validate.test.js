    ...
    describe('Check payload object:', () => {
        ...
        describe('Check payload name field:', () => {
            test('Throw error if payload.name doesnt exist', () => {
                expect(() => validate({data: { payload: { email: 'email', password: 'password'}}, name: 'someName'})
                ).toThrowError('someName: payload should have required field name')
            })

            test('throw error if payload.name empty string', () => {
                expect(() => validate({ data: { payload: { name: '', email: 'email', password: 'password'}}, name: 'someName' })
            ...
            test('throw error if type of payload.name not string', () => {
                expect(() =>
                    validate({
                        data: { payload: { name: 2, email: 'email', password: 'password' } },
                        name: 'someName',
                    }),
                ).toThrowError('someName: payload.name should be a string');
            })
        })
